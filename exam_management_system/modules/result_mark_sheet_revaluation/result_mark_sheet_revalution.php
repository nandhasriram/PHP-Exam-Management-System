...
<div class="container">
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $query = "INSERT INTO cms_revaluation_marksheet (name, date_of_birth, gender, date_of_exam, college, program_branch, regulation, semester_no, course_code, course_title, existing_grade, revaluation_grade, grade_point, result) 
                  VALUES ('$name', '$date_of_birth', '$gender', '$date_of_exam', '$college', '$program_branch', '$regulation', '$semester_no', '$course_code', '$course_title', '$existing_grade', '$revaluation_grade', '$grade_point', '$result')";
    ...
    <h2 class="mt-5">Revaluation Mark Sheets</h2>
    <table class="table table-bordered">
        ...
        <tbody>
            <?php
            $query = "SELECT * FROM cms_revaluation_marksheet";
            ...
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                    ...
                    <td>{$row['existing_grade']}</td>
                    <td>{$row['revaluation_grade']}</td>
                    <td>{$row['grade_point']}</td>
                    <td>{$row['result']}</td>
                    <td>
                    <a href='result_mark_sheet_revalution_edit.php?id={$row['id']}' class='btn btn-warning btn-sm'>Edit</a>
                    </td>
                </tr>";
            }
            ?>
