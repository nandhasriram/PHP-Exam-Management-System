...
// Fetch scholarship records from the `scholarships` table using PDO
$query = "SELECT * FROM scholarships";
$stmt = $conn->prepare($query);
$stmt->execute();
$scholarship_records = $stmt->fetchAll(PDO::FETCH_ASSOC);  // Fetch all records
?>
...
<div class="container mt-5">
    ...
    <?php if (isset($_GET['success']) && $_GET['success'] === 'RecordDeleted'): ?>
        <div class="alert alert-success" role="alert">
            Scholarship record deleted successfully.
        </div>
    <?php elseif (isset($_GET['error']) && $_GET['error'] === 'RecordNotDeleted'): ?>
        <div class="alert alert-danger" role="alert">
            Error deleting the scholarship record.
        </div>
    <?php endif; ?>

    <!-- Check if scholarship records exist -->
    <?php if (empty($scholarship_records)): ?>
            ...
            <tbody>
                <?php foreach ($scholarship_records as $record): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($record['eligibility_criteria']); ?></td>
                        <td><?php echo htmlspecialchars($record['amount']); ?></td>
                        <td><?php echo htmlspecialchars($record['deadline']); ?></td>
                        <td>
                            <!-- Edit Button -->
                            <a href="edit_scholarship.php?id=<?php echo urlencode($record['id']); ?>" class="btn btn-outline-primary">
                                <i class="fas fa-edit"></i> Edit
                            </a>
                            <!-- Delete Button -->
                            <a href="delete_scholarship.php?id=<?php echo urlencode($record['id']); ?>" class="btn btn-outline-danger" onclick="return confirm('Are you sure you want to delete this record?');">
